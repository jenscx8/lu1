...
import { Instructor, Lesson, Student } from "../model.js";

const router = express.Router();

// create new lesson
router.post("/new-lessonn", async (req, res) => {
  ...
  const { lessonType, date } = req.body;
...
router.post("/:lessonId/add-student", async (req, res) => {
...
router.post("/:lessonId/remove-student", async (req, res) => {
  const { lessonId } = req.params;
  ...
  try {
    ...
    lesson.roster = lesson.roster.filter(
    ...
    res.json({
      message: "Student removed from lesson roster",
