...
router.post("/edit", instructorLoginRequired, async (req, res) => {
  ...
  try {
    ...
    instructor.certification = parsedCertification.length
      ? parsedCertification
...
router.get("/dashboard", instructorLoginRequired, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch dashboard", details: error.message });
  }
});
...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create instructor", details: error.message });
  }
});
...
router.get("/:instructorId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch instructor", details: error.message });
  }
});
